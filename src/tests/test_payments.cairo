...
fn test_successful_handle_order() {
    ...
    store(
        ...
        map_entry_address(
            ...
            array![order_hash_2].span() // map key
        ),
        array![2, 1].span(),
    );
    ...
    store(
        contract_address,
        map_entry_address(selector!("fulfillment"), array![order_hash_3].span()),
        array![0, 10].span(),
    );

    assert_eq!(dispatcher.get_order_fulfillment(order_hash_1), FulfilledStatus::Canceled(0));
    assert_eq!(dispatcher.get_order_fulfillment(order_hash_2), FulfilledStatus::Canceled(1));
    assert_eq!(
        dispatcher.get_order_fulfillment(order_hash_3), FulfilledStatus::PartialFulfilled(10),
    );

    cheat_caller_address_once(:contract_address, caller_address: testing_constants::OPERATOR);
    dispatcher.cancel_orders(order_hashes: array![order_hash_3].span());
    assert_eq!(dispatcher.get_order_fulfillment(order_hash_1), FulfilledStatus::Canceled(0));
    assert_eq!(dispatcher.get_order_fulfillment(order_hash_2), FulfilledStatus::Canceled(1));
    assert_eq!(dispatcher.get_order_fulfillment(order_hash_3), FulfilledStatus::Canceled(10));
}
