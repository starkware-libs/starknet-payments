...
pub impl HashOrderImpl<S, +HashStateTrait<S>, +Drop<S>> of Hash<Order, S> {
    fn update_state(mut state: S, value: Order) -> S {
        let Order {
            salt, expiry, owner, sell_token, buy_token, sell_amount, buy_amount, allowed_addresses,
        } = value;
        state = state
            .update_with(salt)
            .update_with(expiry)
...
const ORDER_TYPE_HASH: HashType = 0x12ba33bf9b3d33faac5f1346b708e16ff135408818b2152ab07b70f1fcea224;
...
mod tests {
    ...
    fn test_order_type_hash() {
        let expected = selector!(
            "\"Order\"(\"salt\":\"felt\",\"expiry\":\"Timestamp\",\"owner\":\"ContractAddress\",\"sell_token\":\"ContractAddress\",\"buy_token\":\"ContractAddress\",\"sell_amount\":\"u128\",\"buy_amount\":\"u128\",\"allowed_addresses\":\"Span<ContractAddress>\")\"Timestamp\"(\"seconds\":\"u64\")",
        );
