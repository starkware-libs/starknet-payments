    ...
    pub impl PaymentsImpl of IPayments<ContractState> {
        fn trade(
            ...
            if prev_seller_fulfilled_amount.is_zero() {
                let (r, s) = seller_signature;

                assert_valid_signature(
                    signer: seller_order.maker,
                    hash: seller_order_hash,
                    signature: array![r, s].span(),
                );
            }

            if prev_buyer_fulfilled_amount.is_zero() {
                let (r, s) = buyer_signature;

                assert_valid_signature(
                    signer: buyer_order.maker,
                    hash: buyer_order_hash,
                    signature: array![r, s].span(),
                );
            }
            ...
            assert(
                sell_token.transfer_from(seller_order.maker, fee_recipient, fee_1.into()),
                TRANSFER_FAILED,
            );
            assert(
                buy_token.transfer_from(buyer_order.maker, fee_recipient, fee_2.into()),
                TRANSFER_FAILED,
            );
            ...
            assert(
                sell_token
                    .transfer_from(
                        seller_order.maker, buyer_order.maker, (actual_sell_amount - fee_1).into(),
                    ),
                TRANSFER_FAILED,
            );
            assert(
                buy_token
                    .transfer_from(
                        buyer_order.maker, seller_order.maker, (actual_buy_amount - fee_2).into(),
                    ),
                TRANSFER_FAILED,
            );
        }
